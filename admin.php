...
if ( isSecureConnection() ) @ini_set( 'session.cookie_secure', 1 ); else @ini_set( 'session.cookie_secure', 0 );
...
if ( php_sapi_name() === 'cli' ) {
  ...
  if ( !checkPhpVersion() ) {
    showWarningJson( ['status' => 0, 'message' => sprintf( L( 'PHP 5.6 or newer is required. The version of PHP you\'re running is %s.' ), PHP_VERSION )], 1 );
  }
  if ( !empty( $missingExtensions ) ) {
    showWarningJson( ['status' => 0, 'message' => 'Following extensions are missing: ' . implode( ', ', $missingExtensions )], 1 );
  }
  if ( version_compare( getSqliteVersion(), '3.7.0', '<' ) ) {
    showWarningJson( ['status' => 0, 'message' => 'Your SQLite version is lower than 3.7.0'], 1 );
  }
  if ( empty( $cli['action'] ) ) {
  ...
  if ( $sourcePath === false ) showWarningJson( ['status' => 0, 'message' => 'No website detected'], 1 );
  switch ( $cli['action'] ) {
    ...
    case 'perform.update' :
      updateSystem();
      showWarningJson( ['status' => 1, 'message' => 'Update performed'], 1 );
      break;
    case 'get.info' :
      $dsn                         = getDSN();
      ...
      showWarningJson( ['status' => 1, 'info' => $cliGetInfo], 1 );
      break;
    case 'delete.backups' :
      $dsn = getDSN();
      deleteBackups( ['all' => 1] );
      showWarningJson( ['status' => 1, 'message' => 'Backups deleted'], 1 );
      break;
    case 'delete.urls' :
      $dsn = getDSN();
      cliDeleteUrls( $data );
      break;
    default :
      showWarningJson( ['status' => 0, 'message' => 'No such command'], 1 );
  }
  exit( 0 );
}
...
function cliDeleteUrls( $data )
{
  ...
  if ( empty( $data ) ) return showWarningJson( ['status' => 0, 'message' => ''], 1 );

  $sqlWhere = implode( ' AND ', array_map( function ( $v ) {
    return "{$v} = :{$v}";
  }, array_keys( $data ) ) );
  if ( !strlen( $sqlWhere ) ) return showWarningJson( ['status' => 0, 'message' => ''], 1 );
  ...
  showWarningJson( ['status' => 1, 'message' => 'Delete action performed', 'urls_deleted' => $deletedUrls], 0 );
}
...
function cliPerformImport( $data )
{
  ...
  if ( $sourcePath === false ) {
    showWarningJson( ['status' => 0, 'message' => 'No installed website found'], 1 );
  }
  $ACMS['ACMS_TIMEOUT'] = 0;
  ...
  if ( empty( $import ) ) {
    showWarningJson( ['status' => 0, 'message' => 'Cannot import .ZIP file'], 1 );
  }
  ...
  showWarningJson( ['status' => 1, 'message' => 'Import installed'], 1 );
}
...
function cliPerformInstall( $data, $settings )
{
  ...
  showWarningJson( ['status' => 1, 'message' => 'Website installed'], 1 );
}
...
function cliUpdateAcmsSettings( $settings )
{
  global $sourcePath;
  if ( $sourcePath === false ) {
    showWarningJson( ['status' => 0, 'message' => 'Website is not found.'], 1 );
  }
  setAcmsSettings( $settings );
  showWarningJson( ['status' => 1, 'message' => 'Settings updated.'], 1 );
}
...
function cloneUrl( $input )
{
  ...
  if ( $cloneID ) {
    ...
    copy( $sourcePath . DIRECTORY_SEPARATOR . $metaData['folder'] . DIRECTORY_SEPARATOR . $metaData['filename'], $sourcePath . DIRECTORY_SEPARATOR . $metaData['folder'] . DIRECTORY_SEPARATOR . sprintf( '%s.%08d.%s', convertPathToFilename( $input['request_uri'] ), $cloneID, $cloneFileExtension ) );
    $stmt = $pdo->prepare( 'UPDATE structure SET filename = :filename WHERE rowid = :rowid' );
    $stmt->execute( [
      'filename' => sprintf( '%s.%08d.%s', convertPathToFilename( $input['request_uri'] ), $cloneID, $cloneFileExtension ),
      'rowid'    => $cloneID,
    ] );
...
function getTemplates( $hostname = '' )
{
  ...
  if ( strlen( $hostname ) ) {
    ...
    $stmt = $pdo->query( "SELECT rowid, * FROM templates ORDER BY name" );
  }
  return $stmt->fetchAll( PDO::FETCH_ASSOC );
}
...
function importPerform( $importFileName, $importSettings, $taskOffset = 0 )
{
  ...
  while ( $url = $stmt->fetch( PDO::FETCH_ASSOC ) ) {
    ...
    $url['tmp_file_path'] = tempnam( getTempDirectory(), 'archivarix.' ) . '.tmp';
...
function loadLocalization( $languageCode )
{
  $localization = [
    'ru' => [
      ...
      'cadbf6f5c73c781d4999c3ac81b5da9449b2096d' => 'Обновлено %d неправильно закодированных URL в %d страницах.',
...
function removeBrokenImages( $taskOffset = 0 )
{
  ...
  while ( $url = $stmt->fetch( PDO::FETCH_ASSOC ) ) {
    ...
    for ( $n = $imgTags->length - 1; $n >= 0; --$n ) {
      ...
      if ( !empty( $_POST['stats_only'] ) && !urlExists( $hrefVariants ) ) {
        $stats['stats'][$hrefAbsolute][0] = isset( $stats['stats'][$hrefAbsolute][0] ) ? $stats['stats'][$hrefAbsolute][0] + 1 : 1;
      }
...
function updateMetaFiles( $taskOffset = 0 )
{
  global $taskIncomplete;
  global $taskIncompleteOffset;
  global $taskStats;
...
if ( isset( $_POST['action'] ) &&
  ...
  if ( !inSafeMode() ) {
    switch ( $_POST['action'] ) {
      ...
      case 'fun.convert.website.delete' :
        ...
        $section = 'tools';
        break;
      case 'update.meta.files' :
        $updatedUrls = updateMetaFiles( $taskIncompleteOffset );
  ...
  switch ( $_POST['action'] ) {
    ...
    case 'broken.encoded.urls' :
      ...
      addWarning( sprintf( L( '%d encoded URLs in %d pages are updates.' ), $updatedUrls['links'], $updatedUrls['pages'] ), 1, L( 'Broken URLencoded links' ) );
      $section = 'tools';
      break;
    ...
    case 'api.cloudflare.create' :
      if ( setCloudflareToken( $_POST['zone_id'], $_POST['token'] ) ) addWarning( L( 'Access is successfully saved.' ), 1, 'Cloudflare' );
    ...
    case 'convert.www' :
      updateWebsiteWww();
      addWarning( L( 'Website converted.' ), 1, L( 'Convert to www/non-www' ) );
      $section = 'tools';
      break;
    case 'edit.trackers' :
...
?>
  ...
  <head>
    ...
    <?php if ( in_array( $section, ['table', 'missing.urls', 'backups'] ) ) { ?>
    ...
    <?php } ?>
    <?php if ( in_array( $section, ['settings', 'template', 'htaccess', 'trackers'] ) || isset( $metaData['rowid'] ) ) { ?>
        ...
        <div class="row h-100">
          ...
          <div class="col-12 col-lg-9 ml-auto pt-0 order-1 order-md-2 pb-3" id="main">
            ...
            <div class="row row-cols-1 row-cols-md-3">
                ...
                <div class="col mb-4">
                  <div class="card h-100 shadow">
                    <div class="card-header text-white bg-info">
                      <i class="fas fa-flask fa-fw"></i> <?=L( 'Convert to www/non-www' )?>
                    </div>
                    <div class="card-body small">
                ...
                <div class="col mb-4">
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post">
                        <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                        <input type="hidden" name="action" value="update.meta.files">
                        <input type="hidden" name="urlID" value="<?=!empty( $metaData['rowid'] ) ? $metaData['rowid'] : ''?>"/>
                        <input type="hidden" name="filterValue" class="filterValue" value="<?=htmlspecialchars( $filterValue )?>"/>
                        <input type="hidden" name="urlOffsets" class="urlOffsets" value="<?=htmlspecialchars( serialize( $urlOffsets ) )?>"/>
                        <button type="submit" class="btn btn-sm btn-primary">
              ...
              <?php if ( true && !inSafeMode() && inExpertMode() ) { ?>
              <div class="col mb-4">
                <div class="card h-100 shadow">
                  <form action="" method="post">
                    <div class="card-header text-white bg-info">
                    ...
                    <div class="card-body small">
                      <div class="form-group">
                        <label><?=L( 'Filename' )?></label>
                        <input type="text" name="params[filename]" class="form-control" autocorrect="off" autocapitalize="none">
                      </div>
                      <div class="form-group">
                        <label><?=L( 'Hostname' )?></label>
                        <input type="text" name="params[hostname]" class="form-control" autocorrect="off" autocapitalize="none">
                      </div>
                    </div>
                    <div class="card-footer text-right">
                      <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                            ...
                            <div class="search-filter-block rounded-bottom bg-light text-dark p-0 small mb-3 position-relative ">
                              ...
                              <div class="p-3">
                                <div class="form-row">
                                  ...
                                  <div class="col-12 search-filter-regex-div <?=( in_array( $filterCur['param'], ['datetime', 'filesize'] ) ? 'd-none' : '' )?> mb-1">
                                  ...
                                  <div class="col-12 search-filter-case-sensitive-div <?=( in_array( $filterCur['param'], ['datetime', 'filesize'] ) ? 'd-none' : '' )?> mb-1">
                                    <div class="form-check" style="line-height:1 !important;">
                                      <label class="form-check-label">
                            ...
                            <div class="search-filter-block rounded-bottom bg-light text-dark p-0 small mb-3 position-relative ">
                              ...
                              <div class="p-3">
                                <div class="form-row">
                                  ...
                                  <div class="col-12 search-filter-case-sensitive-div <?=( in_array( $filterCur['param'], ['datetime', 'filesize'] ) ? 'd-none' : '' )?>">
                                    <div class="form-check" style="line-height:1 !important;">
                                      <label class="form-check-label">
                                        <input type="hidden" class="search-filter-case-sensitive" name="filter[case_sensitive][]" value="<?=$filterCur['case_sensitive']?>"/>
                                        <input type="checkbox" class="form-check-input search-filter-case-sensitive-checkbox" style="margin-top: 0 !important;" <?=( $filterCur['case_sensitive'] ? 'checked' : '' )?>>
          ...
          if ( !empty( $missingUrls ) ) { ?>
            ...
            <table data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-export="true" data-page-size="100" data-page-list="[10, 25, 50, 100, 200, All]" data-maintain-meta-data="true" data-locale="<?=$cmsLocales[getLang()]['code']?>" data-sort-class="bg-light" data-sort-name="status" data-sort-order="desc" data-trim-on-search="false" class="table table-bordered table-sm">
              <thead class="thead-light">
              <th data-field="wbm" data-sortable="false" data-visible="false" data-align="center">WBM</th>
              <th data-field="url" data-sortable="true" data-width="100" data-width-unit="%"><?=L( 'URL' )?></th>
                  ...
                  <div class="col-12 col-md-6 small">
                    <table class="table table-sm table-hover mb-3">
                      ...
                      <tr>
                        ...
                        <td><?=getSqliteVersion()?> <?=( version_compare( getSqliteVersion(), '3.7.0', '>=' ) ? '' : '<i class="fas fa-exclamation-circle text-danger" data-toggle="tooltip" title="' . L( 'We have to use a legacy .db file because you have outdated SQLite version. The minimum recommended version is 3.7.0' ) . '"></i>' )?></td>
                      </tr>
                      <tr>
                        <td><?=L( 'PHP version' )?></td>
                      ...
                      <tr>
                        ...
                        <td>
                          <?php
                          $recommendedExtensions = ['curl', 'dom', 'fileinfo', 'iconv', 'intl', 'json', 'libxml', 'pcre', 'pdo_sqlite', 'zip',];
